...
fs.writeFileSync('dist/CamoMarble.user.js', resultTerser.code, 'utf8');
...
if (!isGitHub) {
  try {
    importedMapCSS = JSON.parse(fs.readFileSync('dist/CamoMarble.user.css.map.json', 'utf8'));
  } catch {
...
const mapCSS = mangleSelectors({
  inputPrefix: 'bm-',
  outputPrefix: 'bm-',
  pathJS: 'dist/CamoMarble.user.js',
  pathCSS: 'dist/CamoMarble.user.css',
...
if (mapCSS) {
  fs.writeFileSync('dist/CamoMarble.user.css.map.json', JSON.stringify(mapCSS, null, 2));
}
...
fs.writeFileSync(
  ...
  metaContent + fs.readFileSync('dist/CamoMarble.user.js', 'utf8'), 
  'utf8'
);
